...
// PptxGenJS fill format: { type: 'grad', gradientType: 'lin', angle, stops: [{color, pos, transparency}] }
export function toPptxGradientFill(gradient: GradientDescriptor): object {
  const stops = gradient.stops.map((stop) => ({
    color: stop.color.hex,
    pos: stop.position,                          // PptxGenJS uses 'pos' not 'position'
  ...
  if (gradient.direction === "linear") {
    return {
      type: "grad",
      gradientType: "lin",
      angle: gradient.angle || 0,               // PptxGenJS uses 'angle' not 'rotate'
      stops,
    };
  }
